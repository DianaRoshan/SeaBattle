...
void draw(Game& player1, Game& player2) {
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			...
			player1.sprite.setPosition(width + (i * 32), height + (j * 32));//по сути раскидывает квадратики, превращая в карту. то есть задает каждому из них позицию. 
			//если убрать, то вся карта нарисуется в одном квадрате 32*32 и мы увидим один квадрат
...
int main() {
	...
	Text Titletxt("", font, 20);//создаем объект текста. закидываем в объект строку, шрифт, размер шрифта(в пикселях)
